...
Type& TVector<Type>::GetValue(int pos)
{
	if (pos > Size || pos < StartIndex) 
		throw out_of_range("You cannot reach this element. No such element. Out of range.");
...
{
	TVector<Type> tmp(*this);
	//if (v.Size != tmp.Size || v.StartIndex != tmp.StartIndex) 
		//throw length_error("Vectors' sizes must be equal");
